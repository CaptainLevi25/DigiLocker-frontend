...
export default function Firstpage({
  ...
  const add = async () => {
     sett();
   try{ await contract.addUser();
    nav("/homepage")}catch{nav("/homepage")}
  };
  return (
    <div>
      <div className="cover-page">
        ...
        <div className="content">
          <img
            ...
            alt="Metamask"
          />
          <h1 className="title">Decentralised Digi Locker</h1>
          <p className="description">
            Welcome to the future of cloud storage - a decentralized solution
            for secure and reliable file storage.
          </p><div className="f">

          <button className="get-started-button" onClick={()=>{connect();sett()}}>Connect to Metamask</button>
          <button className="get-started-button"  onClick={() => {
          add();
        
        }}>Add yourself / Access Our features</button>
          </div>
          <div className="scroll-down">Scroll down to get more info</div>
        </div>
      </div>
